...
def majority_vote(completions, extract_ans):
    if not completions or not extract_ans or len(completions) != len(extract_ans):
        raise ValueError("Inputs must be non-empty and of the same length.")

    extract_ans = [_[0] if _ else 'Null' for _ in extract_ans]
    
    answer_counts = Counter(extract_ans)
    majority_answer = answer_counts.most_common(1)[0][0]
    
    for completion, answer in zip(completions, extract_ans):
        if answer == majority_answer:
            return [majority_answer], completion

    raise RuntimeError("No majority answer found.")  # This should not happen
...
def test_hendrycks_math(model, data_path, remainder=0, n_groups=MAX_INT, batch_size=1, tensor_parallel_size=1, args=None):
    ...
    sampling_params = SamplingParams(temperature=args.temp, top_p=args.top_p, n=args.sampling_size, max_tokens=2048)
    print('sampling =====', sampling_params)
    if not os.path.exists(save_path):
        llm = LLM(model=model, tokenizer=args.tokenizer_name, tensor_parallel_size=tensor_parallel_size, dtype=torch.bfloat16, seed=args.seed,max_model_len=2048, gpu_memory_utilization=0.8)
        ...
        for idx, (prompt, prompt_answer) in enumerate(zip(batch_hendrycks_math_ins, hendrycks_math_answers)):
            ...
            for output in completions:
                ...
                generated_text = [ot.text for ot in output.outputs]
    ...
    to_save_list = []
    results = []
    for idx, (prompt, completions, prompt_answer, attribute) in enumerate(zip(hendrycks_math_ins, res_completions, hendrycks_math_answers, attributes)):

        # if isinstance(prompt_answer, str) and prompt_answer.startswith("\\text{"):
        #     prompt_answer = remove_text(prompt_answer)

        if isinstance(prompt_answer, list) and len(prompt_answer) == 1 and "\\begin{pmatrix}" in prompt_answer[0]:
        ...
        for completion in completions:
            if "The answer is:" in completion:
                ...
                extract_ans.append(extract_math_answer(prompt, completion, task='cot'))
            else:
                extract_ans.append(['Null'])

        if args.verifier == "sc":
            final_ans, final_completion = majority_vote(completions, extract_ans)
        else:
            final_ans, final_completion = extract_ans[0], completions[0]

        item = {
            'question': prompt,
            'model_output': final_completion,
            'prediction': final_ans,
            'answer': prompt_answer if isinstance(prompt_answer, list) else [prompt_answer],
        }

        if len(item['prediction']) == 0:
            invalid_outputs.append({'question': prompt, 'output': final_completion, 'answer': item['prediction']})
            res = False
            extract_ans = None
        else:
...
def parse_args():
    ...
    parser.add_argument("--save_path", type=str, default="eval_results/tmp.json")
    parser.add_argument("--tokenizer_name", type=str, default="xinlai/Qwen2-7B-SFT")
    parser.add_argument("--prompt", type=str, default='alpaca')
    parser.add_argument("--temp", type=float, default=0.0)
    parser.add_argument("--top_p", type=float, default=1.0)
    parser.add_argument("--seed", type=int, default=1234)
    parser.add_argument("--rep", type=int, default=1)
    parser.add_argument("--verifier", type=str, default='default')
